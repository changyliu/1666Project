...
GainType Penalty_PDPTW()
{
    ...
    do {
        ...
        do {
            /*printf("cost=" GainFormat ", node=%d, earliest=%f, latest=%f\n", CostSum, N->Id, N->Earliest, N->Latest);*/
            if (N->Id <= Dim && N != Depot) {
                DemandSum += N->Demand;
            ...
            /*printf("TravelTime=%d, N->Pi=%d, NextN->Pi=%d, Precision=%d\n", C(N, NextN), N->Pi, NextN->Pi, Precision);*/
