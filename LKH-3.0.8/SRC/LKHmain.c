...
int main(int argc, char *argv[])
{
    ...
    if (Salesmen > 1) {
        ...
        int name_len = (int)strlen(ProblemFileName)-6;

        char ext[] = "-tour.txt";
        char SaveFileName[name_len+1+(int)strlen(ext)];
        memcpy(SaveFileName, &ProblemFileName[0], name_len);
        SaveFileName[name_len] = '\0';
        strcat(SaveFileName, ext);
        printf("%s\n", SaveFileName);
