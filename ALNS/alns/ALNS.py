...
class ALNS:
    ...
    def iterate(self, initial_solution, weights, operator_decay, criterion,
                iterations=10000, collect_stats=True, time_limit=-1, 
                early_stopping=True, stop_iter=1000, verbose=0):
        """
        ...
        best_iter = 0 # the iteration when the incumbent was found

        time0 = time.time()
        if verbose:
            print("current", current.objective(), current.solution, current.time_window_violation())
        for iteration in range(iterations):
            ...
            if time_limit != -1 and time.time() - time0 > time_limit:
                break
    ...
    def _consider_candidate(self, best, current, candidate, criterion, iteration, verbose=0):
        """
